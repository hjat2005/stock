...
class Mag7HybridStrategy(bt.Strategy):
    ...
    【美股M7周/日混合增强策略】
    逻辑：
    1. 买入前置：5周均线 上穿 12周均线。
    2. 买入触发：连续3日成交量递增。
    3. 卖出前置：5周均线 下穿 12周均线。
    4. 卖出触发：连续3日成交量递减。
    5. 判定时间：模拟收盘前半小时（使用当日收盘价）。
    """
    params = dict(
        w_ma_fast=5,     # 5周线
        w_ma_slow=12,    # 12周线（改为12周）
        vol_days=3,      # 连续3日
        printlog=True,
    )
    ...
    def __init__(self):
        ...
        for d in self.datas:
            ...
            # 日线数据模拟周线均线 (1周=5交易日)
            # w_ma_fast=5 → 5周*5日=25日均线
            # w_ma_slow=12 → 12周*5日=60日均线
            self.inds[d]['w_ma_fast'] = bt.indicators.SMA(d.close, period=self.p.w_ma_fast * 5)
            self.inds[d]['w_ma_slow'] = bt.indicators.SMA(d.close, period=self.p.w_ma_slow * 5)
    ...
    def next(self):
        for d in self.datas:
            ...
            w_fast = self.inds[d]['w_ma_fast'][0]
            w_slow = self.inds[d]['w_ma_slow'][0]
            ...
            if not pos.size:
                ...
                if is_golden_zone and self.inds[d]['vol_up']:
                    ...
                    vol_1d_ago = d.volume[-1]
                    vol_2d_ago = d.volume[-2]
                    vol_3d_ago = d.volume[-3]
                    
                    # 计算增量百分比
                    inc_1 = ((vol_today - vol_1d_ago) / vol_1d_ago * 100) if vol_1d_ago > 0 else 0
                    inc_2 = ((vol_1d_ago - vol_2d_ago) / vol_2d_ago * 100) if vol_2d_ago > 0 else 0
                    inc_3 = ((vol_2d_ago - vol_3d_ago) / vol_3d_ago * 100) if vol_3d_ago > 0 else 0
                    ...
                    if size > 0:
                        ...
                        self.log(f'  └─ 📈成交量连续递增: V-3日={vol_3d_ago:,.0f} → V-2日={vol_2d_ago:,.0f} ({inc_3:+.1f}%) '
                                f'→ V-1日={vol_1d_ago:,.0f} ({inc_2:+.1f}%) → 今日={vol_today:,.0f} ({inc_1:+.1f}%)')
